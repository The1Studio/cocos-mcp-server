...
Cocos Creator MCP Server is a comprehensive Model Context Protocol (MCP) server plugin designed for Cocos Creator 3.8+, enabling AI assistants to interact with the Cocos Creator editor through a standardized protocol.

This document details all available MCP tools and their usage.

## Tool Categories

The MCP server provides **158 tools**, categorized into 13 main categories:
...
**Returns**: Name, UUID, type, active state, and node count of the current scene
...
**Returns**: List of all scenes in the project, including name, path, and UUID
...
Get the complete hierarchy structure of the current scene
...
- `parentUuid` (string, **strongly recommended**): Parent node UUID. **Important**: Strongly recommended to always provide this parameter. Use `get_current_scene` or `get_all_nodes` to find parent node UUID. If not provided, node will be created at scene root.
- `nodeType` (string, optional): Node type, options: `Node`, `2DNode`, `3DNode`, defaults to `Node`
- `siblingIndex` (number, optional): Sibling index, -1 means append to end, defaults to -1

**Important Note**: To ensure nodes are created at the expected location, always provide the `parentUuid` parameter. You can get parent node UUID by:
- Using `scene_get_current_scene` to get scene root node UUID
- Using `node_get_all_nodes` to view all nodes and their UUIDs
- Using `node_find_node_by_name` to find a specific node's UUID
...
Get all nodes and their UUIDs in the scene
...
**Parameters**:
- `uuid` (string, required): UUID of node to delete
...
**Parameters**:
- `nodeUuid` (string, required): UUID of node to move
- `newParentUuid` (string, required): New parent node UUID
- `siblingIndex` (number, optional): Sibling index in new parent, defaults to -1
...
**Parameters**:
- `uuid` (string, required): UUID of node to duplicate
- `includeChildren` (boolean, optional): Whether to include children, defaults to true
...
Add component to specified node

**Parameters**:
- `nodeUuid` (string, **required**): Target node UUID. **Important**: Must specify the exact node to add component to. Use `get_all_nodes` or `find_node_by_name` to get the UUID of the desired node.
- `componentType` (string, required): Component type (e.g., cc.Sprite, cc.Label, cc.Button)

**Important Note**: Before adding a component, ensure you:
1. First use `node_get_all_nodes` or `node_find_node_by_name` to find target node's UUID
2. Verify the node exists and UUID is correct
...
Attach script component to node
...
**⚠️ Known Issues**: When instantiating prefabs using standard Cocos Creator API, complex prefab structures with child nodes may not be correctly restored. While prefab creation correctly saves all child node information, the instantiation process using `create-node` with `assetUuid` has limitations that may cause child nodes to be missing from instantiated prefabs.
...
- `position` (object, optional): Initial position, contains x, y, z properties
...
**⚠️ Functional Limitation**: Complex prefabs with child nodes may not instantiate correctly. Due to limitations in Cocos Creator API when using `assetUuid` with the standard `create-node` method, only the root node may be created and child nodes may be lost. This is a known issue with the current implementation.
...
- `savePath` (string, required): Path to save prefab
...
Create prefab from node (alias of create_prefab)

**Parameters**:
- `nodeUuid` (string, required): Source node UUID
- `prefabPath` (string, required): Path to save prefab
...
Revert prefab instance to original state
...
- `debug` (boolean, optional): Whether debug build, defaults to true
...
Create new asset file or folder
...
Copy asset to another location
...
**Parameters**:
- `limit` (number, optional): Number of recent logs to retrieve, defaults to 100
...
**Parameters**:
- `lines` (number, optional): Number of lines to read from end of log file, defaults to 100, range: 1-10000
- `filterKeyword` (string, optional): Filter logs by specified keyword
...
**Parameters**:
- `pattern` (string, required): Search pattern (supports regular expressions)
- `maxResults` (number, optional): Maximum number of match results, defaults to 20, range: 1-100
- `contextLines` (number, optional): Number of context lines to show around matches, defaults to 2, range: 0-10
...
**Parameters**:
- `limit` (number, optional): Number of recent messages to return, defaults to 50
...
Start listening to specific broadcast message
...
Stop listening to specific broadcast message
...
Cocos Creator uses asset URL format with `db://` prefix:
...
If a tool call fails, an error message is returned:
...
1. **Query before modifying**: Before modifying nodes or components, use query tools to get current state
2. **Use UUIDs**: Use UUIDs instead of names to reference nodes and assets when possible
3. **Check errors**: Always check return values of tool calls to ensure operations succeeded
4. **Asset management**: Before deleting or moving assets, ensure they're not referenced elsewhere
...
2. Use `debug_validate_scene` to check if scene has issues
3. Use `debug_get_editor_info` to get environment information
4. Check MCP server running status and logs
