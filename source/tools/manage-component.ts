...
export class ManageComponent extends BaseActionTool {
        ...
        try {
            ...
            if (allComponentsInfo2.success && allComponentsInfo2.data?.components) {
                ...
                if (addedScript) {
                    ...
                    return errorResult(`Script '${scriptName}' was not found on node after addition. Available components: ${allComponentsInfo2.data.components.map((c: any) => c.type).join(', ')}`);
                }
            } else {
                return errorResult(`Failed to verify script addition: ${allComponentsInfo2.error || 'Unable to get node components'}`);
            }
        } catch (err: any) {
            // Fallback: use scene script
            try {
                ...
                // ignore fallback error, fall through to error response
            }
            return {
                success: false,
    ...
    private async checkAndRedirectNodeProperties(args: any): Promise<ActionToolResult | null> {
        ...
        // Only redirect when user explicitly targets cc.Node as a component type
